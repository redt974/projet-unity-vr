...
public class BoutonFollow : MonoBehaviour
{
    ...
    [Tooltip("Axe local de d�placement")]
    ...
    [Tooltip("Tag utilis� par le collider de l'interactor (pointeur)")]
    ...
    void OnTriggerExit(Collider other)
    {
        if (other.CompareTag(interactorTag))
        {
            ...
            isFollowing = false;
            freeze = false;
            interactable.Highlight(false); // on d�sactive la couleur
        }
    }

    /// <summary>
    /// Peut �tre appel� depuis votre interactor (ex�: dans OnGrab) 
    /// pour bloquer le bouton en position appuy�e.
    /// </summary>
